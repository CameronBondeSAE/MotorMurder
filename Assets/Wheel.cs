...
public class Wheel : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Physics.Raycast(ray, out raycastHit, suspensionLength))
		{
			...
			mainBody.AddForceAtPosition(((myTransform.up * suspensionStrength) * (suspensionLength - raycastHit.distance)) * (1 - damping), myTransform.position);

			// Lateral friction (basically just push in the opposite direction to our sideways velocity)
			mainBody.AddForceAtPosition(myTransform.TransformDirection(new Vector3(-localVelocity.x * lateralFriction, 0, 0)), myTransform.position);
		}
		else
			inAir = true;
